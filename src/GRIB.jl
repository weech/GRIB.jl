...
function __init__()
    ...
    if Sys.iswindows()
        ...
        definitions = joinpath(Base.@__DIR__, "..", "definitions") * ";" * joinpath(share, "definitions")
        ctx = ccall((:codes_context_get_default, eccodes), Ptr{codes_context}, ())
        ccall((:codes_context_set_definitions_path, eccodes), Cvoid, (Ptr{codes_context}, Cstring), ctx, definitions)
    else
